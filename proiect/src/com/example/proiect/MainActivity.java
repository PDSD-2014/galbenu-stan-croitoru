...
final EditText verificare = (EditText)findViewById(R.id.verificare);
		...
		//final TranslateAnimation ta = new TranslateAnimation(0, convertDpToPixel(54, getBaseContext()), 0, 0);
		...
		RelativeLayout.LayoutParams lpar = (android.widget.RelativeLayout.LayoutParams) t[0][0].getLayoutParams();
			...
			public void LeftRightSwipe(){
				...
				for(i = 0; i < 4; i++)
					for(j = 2; j >= 0; j--)
					{
						...
						if(m[i][j + moves[i][j]] != 0)
							...
							m[i][j + moves[i][j]] = m[i][j] + m[i][j + moves[i][j]];
							m[i][j] = 0;
						}
					}
				...
				for(i = 0; i < 4; i++)
					for(j = 0; j < 3; j++)
					{
						if (moves[i][j] == 0)
							continue;
						ta[i][j] = new TranslateAnimation(0, moves[i][j]*slide_dist + moves[i][j]*margin, 0, 0);
				...
				if((firsti != -1) && (firstj != -1))
						ta[firsti][firstj].setAnimationListener(new Animation.AnimationListener() {
						@Override
			...
			public void UpDownSwipe(){
					...
					for(j = 0; j < 4; j++)
					{
						if((moves[i][j] != 0) && (m[i][j] != 0))
							...
							if(m[i + moves[i][j]][j] != 0)
								...
								m[i + moves[i][j]][j] = m[i][j] + m[i + moves[i][j]][j];
								m[i][j] = 0;
							}
					}
				...
				for(i = 0; i < 3; i++)
					for(j = 0; j < 4; j++)
					{
						if (moves[i][j] == 0)
							continue;
						ta[i][j] = new TranslateAnimation(0, 0, 0, moves[i][j]*slide_dist + moves[i][j]*margin);
						ta[i][j].setDuration(duration);
						t[i][j].startAnimation(ta[i][j]);
						if((firsti == -1) && (firstj == -1))
						{
						...
						/*public void onAllAnimationEnd() {
							...
							for(i = 0; i < 4; i++)
								for(j = 0; j < 4; j++)
								{
									if (moves[i][j] == 0)
										continue;
									if((ta[i][j].hasStarted()) && (!ta[i][j].hasEnded()))
										allended = false;
								}
							if(allended == true)
							{
