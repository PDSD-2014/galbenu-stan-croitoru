...
public class MainActivity extends Activity {
	...
	protected void onStart() {
		...
		//TextView r = (TextView) findViewById(R.id.mark);
		//ColorDrawable cd = (ColorDrawable) r.getBackground();
			...
			public void LeftRightSwipe(){
				...
				for(i = 0; i < 4; i++)
					for(j = 0; j < 3; j++)
					{
						if (moves[i][j] == 0)
							continue;
						ta[i][j] = new TranslateAnimation(0, moves[i][j]*slide_dist, 0, 0);
			...
			public void UpDownSwipe(){
				...
				for(i = 0; i < 3; i++)
					for(j = 0; j < 4; j++)
					{
						if (moves[i][j] == 0)
							continue;
						ta[i][j] = new TranslateAnimation(0, 0, 0, moves[i][j]*slide_dist);
						ta[i][j].setDuration(duration);
						t[i][j].startAnimation(ta[i][j]);
						if((firsti == -1) && (firstj == -1))
						{
